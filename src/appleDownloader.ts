...
export class AppleDownloader {
   ...
   * 从Apple开发者网站获取示例项目
   */
  async getAppleProjects(): Promise<AppleProject[]> {
    try {
      console.log('正在获取Apple示例项目...');
      
      const projects: AppleProject[] = [];
   ...
   * 获取Apple项目的下载链接
   */
  async getAppleProjectDownloadUrl(project: AppleProject): Promise<string | null> {
    try {
   ...
   * 下载Apple项目
   */
  async downloadAppleProject(project: AppleProject): Promise<boolean> {
    try {
   ...
   * 下载Apple项目ZIP文件（不解压）
   */
  private async downloadAppleProjectZip(project: AppleProject, downloadUrl: string): Promise<boolean> {
    try {
      const projectDir = path.join(this.downloadDir, project.name);
      if (!fs.existsSync(projectDir)) {
   ...
   * 下载所有Apple项目
   */
  async downloadAllAppleProjects(): Promise<{ success: number; total: number }> {
